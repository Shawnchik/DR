def flow_balance_c_ct_cl():
    import numpy as np
    import Branch0
    import Branch10
    import Branch01
    # 冷却水系統の流量バランス(冷却塔利用時、冷房運転時)
    ...
    global KrPpVlvCT,KrPpCnTR1,KrCnTR1
    ...
    if INVCDP1 == 0 and INVCDP2 == 0 and INVCDP3 == 0 and INVCDP4 == 0:
        ...
        cnt = 0
        while GCnCT1 + GCnCT2 + GCnCT3 + GVlvCT - GCDP1 - GCDP2 - GCDP3 - GCDP4 > 0.1 or \
                GCnCT1 + GCnCT2 + GCnCT3 + GVlvCT - GCDP1 - GCDP2 - GCDP3 - GCDP4 < - 0.1:
            ...
            if NmCT == 1:
                ...
                [GCnCT2,_] = Branch01.branch01(dPCnCT1,KrPpCT2,0,KrCnCT2,1,2846,20)
                [GCnCT3,_] = Branch01.branch01(dPCnCT1,KrPpCT3,0,KrCnCT3,1,2846,20)

            # GVlvCTを求める
            [GVlvCT,_] = Branch01.branch01(dPCnCT1,KrPpVlvCT,0,0,VlvCT,25000,100)

            # GCDP1~4,GCDP1~4を求める
            [_,GCDP1,_,PCDP1] = Branch10.branch10(dPCnCT1,INVCDP1,1,2846,0,121,c0CDP1,c1CDP1,c2CDP1,c3CDP1,c4CDP1,KrPpCnTR1,KrCnTR1)
            ...
            # �e�}�̗��ʂ��ނ荇���܂œ񕪖@
            if GCnCT1 + GCnCT2 + GCnCT3 + GVlvCT - GCDP1 - GCDP2 - GCDP3 - GCDP4 > 0:
                GCnCT1max = GCnCT1
            elif GCnCT1 + GCnCT2 + GCnCT3 + GVlvCT - GCDP1 - GCDP2 - GCDP3 - GCDP4 < 0:
                GCnCT1min = GCnCT1

            FlgError5 = 0
            # 各枝の流量が釣り合うまで二分法
            for i in range(10,2,-1):
                GCnCT100[i] = GCnCT100[-1]
            GCnCT100[1] = GCnCT1
